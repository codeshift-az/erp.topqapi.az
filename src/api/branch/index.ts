...
export const getBranches = async (filter: BranchFilter): Promise<PaginationResult<Branch>> => {
  const { data } = await axios.get(getUrlWithFilter(URL.BRANCH_LIST_URL, filter));
  return data;
};
