...
// Actions
import { createSupplier, updateSupplier } from "@/store/actions";
...
const FormModal = ({ data, show, isEdit, toggle, handleSubmit }: Props) => {
  ...
  const { status, errors } = useSelector((state: RootState) => state.supplier);
  ...
  useEffect(() => {
    if (show && status) {
      if (
        ((isEdit && status.lastAction === updateSupplier.typePrefix) ||
          (!isEdit && status.lastAction === createSupplier.typePrefix)) &&
        status.success
      ) {
