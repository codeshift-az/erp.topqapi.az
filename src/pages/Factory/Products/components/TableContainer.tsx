...
// Actions
import { getFactoryProducts } from "@/store/actions";

interface Props {
  onCreate?: () => void;
  onUpdate?: (data: FactoryProduct) => void;
  onDelete?: (data: FactoryProduct) => void;
}
...
const TableContainer = ({ onCreate, onUpdate, onDelete }: Props) => {
  ...
  const { update, items, status, count } = useSelector((state: RootState) => state.factoryProduct);

  const fetchItems = () => {
    dispatch(getFactoryProducts({ ...filters, page, limit, ordering }));
  };
  ...
  // Columns
  const columnHelper = createColumnHelper<FactoryProduct>();
      ...
      cell: (cell) => {
        return (
          <div className="d-flex gap-3">
            {onUpdate && (
              <Fields.EditButton onClick={() => onUpdate(cell.row.original as FactoryProduct)} />
            )}
            {onDelete && (
              <Fields.DeleteButton onClick={() => onDelete(cell.row.original as FactoryProduct)} />
            )}
          </div>
        );
      },
        ...
        <Card>
          <CardBody>
            <DataTable
              ...
              loading={status.loading && status.lastAction === getFactoryProducts.typePrefix}
              // Pagination
