...
const TableContainer = ({ onCreate, onUpdate, onDelete }: Props) => {
  ...
  const { update, items, status, count } = useSelector((state: RootState) => state.branch);
      ...
      cell: (cell) => {
        return (
          <div className="d-flex gap-3">
            {onUpdate && (
              <Fields.EditButton onClick={() => onUpdate(cell.row.original as Branch)} />
            )}
            {onDelete && (
              <Fields.DeleteButton onClick={() => onDelete(cell.row.original as Branch)} />
            )}
          </div>
        );
      },
        ...
        <Card>
          <CardBody>
            <DataTable
              ...
              loading={status.loading && status.lastAction === getBranches.typePrefix}
              // Pagination
