...
// Actions
import { createProduct, getCategories, updateProduct } from "@/store/actions";
...
const FormModal = ({ data, show, isEdit, toggle, handleSubmit }: Props) => {
  ...
  const { status, errors } = useSelector((state: RootState) => state.product);
  ...
  useEffect(() => {
    if (show && status) {
      if (
        ((isEdit && status.lastAction === updateProduct.typePrefix) ||
          (!isEdit && status.lastAction === createProduct.typePrefix)) &&
        status.success
      ) {
