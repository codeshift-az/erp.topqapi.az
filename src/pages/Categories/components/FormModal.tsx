...
const FormModal = ({ data, show, isEdit, toggle, handleSubmit }: Props) => {
  const title = isEdit ? "Kateqoriya məlumatlarını redaktə et" : "Kateqoriya əlavə Et";

  const { status, errors } = useSelector((state: RootState) => state.category);
  ...
  useEffect(() => {
    if (show && status) {
      if (
        ((isEdit && status.lastAction === updateCategory.typePrefix) ||
          (!isEdit && status.lastAction === createCategory.typePrefix)) &&
        status.success
      ) {
