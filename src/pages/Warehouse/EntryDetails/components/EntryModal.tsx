...
// Actions
import { getSuppliers, updateWarehouseEntry } from "@/store/actions";
...
const EntryModal = ({ data, show, toggle, handleSubmit }: Props) => {
  const title = "Giriş məlumatlarını redaktə et";
  ...
  const validation = useFormik({
    ...
    onSubmit: (values) => {
      ...
      // Invoice
      if (!data || values["invoice"] !== data["invoice"])
  ...
  useEffect(() => {
    if (show && status) {
      if (status.lastAction === updateWarehouseEntry.typePrefix && status.success) {
